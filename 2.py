...
def do_record(channel_id, file_path):
    ...
    length = 5  # 时长, 单位秒
    record_data = sd.rec(frames=length * sample_rate, samplerate=sample_rate, channels=1,
                         blocking=True)  # blocking=True, 能够让录音直到时长
    print(record_data)
...
if __name__ == "__main__":
    ...
    preliminary_instruction()
    output_id = get_output_device_id_by_name("立体声混音 (Realtek High Definition ")
    print(output_id)
    data = read_data("20170001P00001A0006.wav", 1)
    do_record(output_id, "sound.wav")
