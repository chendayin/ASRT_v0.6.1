...
def recognition(x):
    text = client.asr(get_file_content(AUDIO_FILE + x + '.wav'), 'wav', RATE, {
        'dev_pid': DEV_PID,
    })
    return ''.join(text['result']).strip('。')


def save_buffer_data2wav(buff_data, x):
    # 创建pyAudio对象
    p = pyaudio.PyAudio()
    # 打开用于保存数据的文件
    path = AUDIO_FILE + x + '.wav'
...
def main():
    ...
    while running:
        time.sleep(5)
        x = hashlib.md5(str(time.time()).encode()).hexdigest()
        save_buffer_data2wav(b''.join(record._frames), x)
        record._frames = []
        try:
            txt = recognition(x)
            print(txt, end='')
            with open(AUDIO_FILE + x + '.txt', 'w', encoding="utf8") as f:
                f.write(txt)
